...
import { DialogCrearMisOrdenesServicios } from './dialog-crear-misOrdenesServicio/dialog.crear-misOrdenesServicio.component';
...
@Component({
  ...
  imports: [
    ...
    UiBotonComponent,
    DialogCrearMisOrdenesServicios,
...
export class MisAsignacionesComponent implements OnInit {
  ...
  private agenteEnlaceService  = inject(AgenteLegacyService);

  visualizarDetallesModal = signal<boolean>(false);
  visualizarModalCrear    = signal<boolean>(false);
  ...
  agentesLoaded = signal<boolean>(false);

  // ============================================================
  // FILTROS (propiedades regulares para compatibilidad con ngModel)
  // ============================================================
  filtroFolio           = '';
  filtroEstado          = '';
  ...
  // Lista de agentes para los selects
  ...
  // ============================================================
  // KPIs – vienen del resumen devuelto por la API
  // ============================================================
  ...
  private cargarCatalogoAgentesYOrdenes(): void {
    ...
    this.agenteEnlaceService.getAllAgentesMap().subscribe({
      next: (map) => {
        this.agentesMap.set(map);
        this.agentesLoaded.set(true);
        this.cargarOrdenes();
      },
      error: () => {
        this.agentesLoaded.set(true);
  ...
  /** Convierte "yyyy-MM-dd" (input[type=date]) al formato "dd/MM/yyyy" que espera la API */
  ...
  abrirModalCrear(): void { this.visualizarModalCrear.set(true); }
  onOrdenCreada():   void { this.cargarOrdenes(); }
  ...
  getNombreAgentePrincipal(orden: OrdenServicioPruebaItem): string {
    if (!this.agentesLoaded()) return 'Cargando...';
    const id = Number(orden.idAgentePrincipal ?? orden.agentePrincipal);
    if (!id) return 'Sin agente asignado';
    return this.agentesMap()[id] || 'Agente no encontrado';
  }

  getNombresAgentesAuxiliares(agenteAuxiliar: string | null | undefined): { nombre: string; iniciales: string }[] {
    if (!agenteAuxiliar) return [];
    if (!this.agentesLoaded()) return [];
    const map = this.agentesMap();
    return agenteAuxiliar
      ...
      .map(id => {
        const nombre = map[id] || 'Agente no encontrado';
